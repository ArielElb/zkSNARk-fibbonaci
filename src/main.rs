...
fn input_number<F: PrimeField>(message: &str) -> F  where <F as FromStr>::Err: std::fmt::Debug {
    println!("{}", message);
...
fn should_verify_fibonacci_circuit_groth16(a: Fr, b: Fr, numb_of_steps: usize) -> bool {
    ...
    eprintln!("Proving time: {}.{:03} seconds", prove_elapsed_time.as_secs(), prove_elapsed_time.subsec_millis());
    ...
    eprintln!("Verification time: {}.{:03} seconds", verify_elapsed_time.as_secs(), verify_elapsed_time.subsec_millis());
    // Print the final value of fi
...
fn main() {
    ...
    println!("Total time taken: {}.{:03} seconds", elapsed_time.as_secs(), elapsed_time.subsec_millis());
}